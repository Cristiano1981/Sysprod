...
import br.com.sysprod.model.TableCurso;
import br.com.sysprod.update.UpdateCurso;
 ...
 */
public final class BrowseCurso extends javax.swing.JPanel implements FormPainel {

    private List<Curso> listaCursoCodigo = new ArrayList<Curso>();
    private List<Curso> listaCursoNome = new ArrayList<Curso>();
    private Curso curso;
    ...
    public void initComponent() {
        tbCodigo.setToolTipText("Lista de curso cadastrados");
        tbNome.setToolTipText("Lista de curso cadastrados");
    ...
    @Override
    public void novo() {
        UpdateCurso upCurso = new UpdateCurso(null, true);
        upCurso.novo();
        upCurso.setVisible(true);
        filtro();
        limpa();
    }
    ...
    public void filtro() {
        ...
        if (painel.getSelectedIndex() == 1) {
            listaCursoNome.clear();
            if (!nome.trim().equals("")) {
                filtro = "AND descricao like '" + nome + "%'";
            }
        } else {
            listaCursoCodigo.clear();
        }
        if (status != 2) {
            ativo = status == 0;
        ...
        try {
            lista = cursoBD.queryAll(filtro, order);
        } catch (Exception e) {
            e.printStackTrace();
            ErrorVerification.ErrDetalhe(e, "Erro ao buscar lista de curso");
        }

        if (lista != null && !lista.isEmpty()) {
            listaCursoCodigo.addAll(lista);
            listaCursoNome.addAll(lista);
        }
        atualizaTabela();
    }
    ...
    private void codeSelected() {
        ...
        for (int i = 0; i < listaCursoCodigo.size(); i++) {
            if (codigo.equals(listaCursoCodigo.get(i).getId())) {
    ...
    private void initComponents() {
        ...
        tbCodigo.setModel(new TableCurso(listaCursoCodigo));
        ...
        tbNome.setModel(new TableCurso(listaCursoNome));
        ...
        tbCodigo.getSelectionModel().addListSelectionListener(new ListSelectionListener() {
            ...
            public void valueChanged(ListSelectionEvent e) {
                ...
                if (!lsm.isSelectionEmpty()) {
                    ...
                    CursoBD curBD = new CursoBD();
                    try {
                        setCurso(curBD.find(codSelected));
                    } catch (SQLException ex) {
                        ex.printStackTrace();
                        ErrorVerification.ErrDetalhe(ex, "Erro ao buscar curso por código.");
                    }
                }
            }
        });
            ...
            public void valueChanged(ListSelectionEvent e) {
                ...
                if (!lsm.isSelectionEmpty()) {
                    ...
                    try {
                        ...
                        ex.printStackTrace();
                        ErrorVerification.ErrDetalhe(ex, "Erro ao buscar curso por nome.");
                    }
                }
            }
        });
